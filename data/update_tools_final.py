...
def trim_to_target(text, target=1500):
    ...
    if len(words) <= target:
        return text
    # Try to find a good stopping point (end of paragraph or section)
    # But for now, just take the first 'target' words and ensure we don't end mid-sentence
    trimmed_words = words[:target]
    trimmed_text = " ".join(trimmed_words)
    # Basic cleanup to avoid cutting mid-sentence if possible
    last_dot = trimmed_text.rfind('.')
    if last_dot > target * 4: # safety check
...
# Print confirmation
print(f"Ideogram: {count_words(ideogram_content)} words")
print(f"Leonardo AI: {count_words(leonardo_content)} words")
print(f"QuillBot: {count_words(quillbot_content)} words")
print("Updated tools_en.json successfully.")
